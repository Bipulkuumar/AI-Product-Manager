...
if df is not None:
    ...
    if analyse or st.session_state.get('analysed'):
        ...
        c2.metric("😊 Positive", f"{pos_count}", f"{int(pos_count/safe_total*100)}%")
        c3.metric("😐 Neutral", f"{neu_count}", f"{int(neu_count/safe_total*100)}%")
        c4.metric("😞 Negative", f"{neg_count}", f"{int(neg_count/safe_total*100)}%")
